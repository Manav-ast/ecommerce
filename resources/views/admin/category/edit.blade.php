    ...
    <div class="flex justify-center items-center min-h-screen">
        <div class="bg-white shadow-lg rounded-lg p-6 w-full max-w-md">
            ...
            <form id="editCategoryForm" action="{{ route('admin.categories.update', $category->id) }}" method="POST"
                enctype="multipart/form-data" class="space-y-3" onsubmit="return false;">
                @csrf
                @method('PUT')
                ...
                <!-- Success Message -->
                <p id="successMessage" class="text-green-600 font-semibold text-center hidden mt-2">Category updated
                    successfully!</p>
            </form>
        </div>
    </div>
                    ...
                    error: function(xhr) {
                        ...
                        if (xhr.status === 422) {
                            ...
                            for (const field in errors) {
                                $("#error-" + field).html(errors[field][0]).removeClass(
                                    'hidden');
                            }
                        } else {
                            // Show error toast
                            showErrorToast("Something went wrong while updating the category.");
                        }
                    }
                });
            });
